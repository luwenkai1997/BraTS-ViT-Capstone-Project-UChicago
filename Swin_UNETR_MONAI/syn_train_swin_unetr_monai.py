...
labeled_dataset_base_dir = '/home/cc/swin_unetr_train/Synthetic_dataset_random_labels/'  # <--- MODIFY THIS!
...
output_folder_name = 'SwinUNETR_Output_Patch96_Syn'
...
modality_file_suffixes = {
    "t1c": "-scan_t1ce.nii.gz",
    "t1n": "-scan_t1.nii.gz",
    "t2f": "-scan_flair.nii.gz",
    "t2w": "-scan_t2.nii.gz",
}
...
for epoch in range(start_epoch, MAX_EPOCHS):
    ...
    # --- NEW: Add this block to save a periodic checkpoint ---
    if (epoch + 1) % SAVE_CHECKPOINT_FREQ == 0 or (epoch + 1) == MAX_EPOCHS:
        checkpoint_path = os.path.join(checkpoint_dir, f"checkpoint_epoch_{epoch+1}.pth")

        # Gather all necessary data to resume training
        checkpoint_data = {
            'epoch': epoch + 1,
